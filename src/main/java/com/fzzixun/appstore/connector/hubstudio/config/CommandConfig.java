package com.fzzixun.appstore.connector.hubstudio.config;

public class CommandConfig {
    // 应用id
    public static final String APP_ID = "202208111007337465826111488";
    // 应用私钥
    public static final String PRIVATE_KEY = "";
    // hubstudio连接器程序路径
    public static final String CONNECTOR_PATH = "D:\\Hubstudio\\2.15.0.6\\hubstudio_connector.exe";
    // hubstudio程序路径，不填写则默认从连接器路径查找
    public static final String HUBSTUDIO_PATH = "D:\\Hubstudio\\2.15.0.6\\Hubstudio.exe";
    // webdriver程序路径
    public static final String WEBDRIVER_PATH_100 = "chromedriver100.exe";
    public static final String WEBDRIVER_PATH_105 = "chromedriver105.exe";

    // 连接端口
    public static final int SOCKET_PORT = 6873;

    // socket读取数据超时时间（毫秒）
    public static final int SOCKET_SO_TIMEOUT = 60 * 1000;
    // socket连接超时时间（毫秒）
    public static final int SOCKET_CONNECT_TIMEOUT = 10 * 1000;
}
